...
//!
//! Sourced with attribution from:
//! https://github.com/probablyABug/escrow-contract (forked from Goldii-locks/escrow-contract)
//! Dual-licensed under MIT / Apache-2.0.
//!
//! Includes a fix for the **Unfunded Dispute Vulnerability** discovered during
//! invariant testing: in the original implementation, `raise_dispute` and
//! `resolve_dispute` lacked a `meta.funded` check, allowing disputes and refund attempts
//! on escrows before any funds were deposited.
...
// ─────────────────────────────────────────────────────────────────────────────
// Hardened MilestoneEscrow Contract (Fixed)
// ─────────────────────────────────────────────────────────────────────────────
...
impl MilestoneEscrow {
    ...
    /// Fixed `raise_dispute`: checks `meta.funded` before allowing dispute.
    pub fn raise_dispute(
        ...
        // SECURITY FIX: Escrow must be funded before raising a dispute!
        if !meta.funded {
            return Err(Error::NotFunded);
        }
    ...
    /// Fixed `resolve_dispute`: checks `meta.funded` before resolving and transferring.
    pub fn resolve_dispute(
        ...
        // SECURITY FIX: Cannot resolve an unfunded escrow!
        if !meta.funded {
            return Err(Error::NotFunded);
        }
...
// ─────────────────────────────────────────────────────────────────────────────
// Unfixed MilestoneEscrow Contract (Vulnerable to Unfunded Dispute)
// ─────────────────────────────────────────────────────────────────────────────

pub mod unfixed {
    use super::*;

    /// Legacy / vulnerable version reproducing the exact unfunded dispute behavior
    /// from the original upstream repository.
    #[contract]
    pub struct UnfixedMilestoneEscrow;

    #[contractimpl]
    impl UnfixedMilestoneEscrow {
        #[allow(clippy::too_many_arguments)]
        pub fn initialize(
            env: Env,
            admin: Address,
        ...
        pub fn fund(env: Env, client: Address) -> Result<(), Error> {
            MilestoneEscrow::fund(env, client)
        }

        /// Flawed raise_dispute: does NOT check `meta.funded`.
        pub fn raise_dispute(
            env: Env,
            caller: Address,
            milestone_index: u32,
            ...
            caller.require_auth();
            let meta = MilestoneEscrow::load_job_meta(&env)?;

            if meta.client != caller && meta.freelancer != caller {
                return Err(Error::Unauthorized);
            }

            // BUG: Missing `if !meta.funded` check!
            let mut milestone = MilestoneEscrow::load_milestone(&env, milestone_index)?;
            ...
            milestone.status = MilestoneStatus::Disputed;
            MilestoneEscrow::store_milestone(&env, milestone_index, &milestone);
            Ok(())
        }
        ...
        pub fn resolve_dispute(
            ...
            MilestoneEscrow::resolve_dispute(env, arbiter, milestone_index, release_to_freelancer)
        }
        ...
        pub fn approve_milestone(
            ...
            MilestoneEscrow::approve_milestone(env, client, milestone_index)
        }

        pub fn get_job(env: Env) -> Result<Job, Error> {
            MilestoneEscrow::get_job(env)
        }
    }
}
